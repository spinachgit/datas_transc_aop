...
public class BaseDaoImpl extends HibernateDaoSupport implements BaseDao {
	...
	public PageResult listBySqlPage(final String sql, PageResult PageResult) throws Exception{
		...
		List ls=this.getHibernateTemplate().executeFind(
				new HibernateCallback() {
	...
	public PageResult pagedQueryByNameParam(String hql, PageResult page,
		...
		List countlist = this.getHibernateTemplate().findByNamedParam(
				countQueryString, paramNames, values);
	...
	public  String assemblingSQL(String tableName,String sql,Long accountId) {
		String tempSQL="SELECT * FROM " +
				"("+sql+") tempTable_oms," +
				"TM_DATA_AUTHORITY tempAuthority_oms " +
				"WHERE tempAuthority_oms.RES_ID=tempTable_oms.ID " +
				"AND tempAuthority_oms.PATH='"+tableName+"' "+
	...
	private static String assemblingQuery(String tempHQL,String objName,String className,Long accountId,String temp){
		tempHQL=tempHQL+" "+temp+" "+objName+".id in (select tempAuthority_oms.resId " +
		"from TmDataAuthority tempAuthority_oms " +
		"where tempAuthority_oms.objName='"+className+"'"+" and tempAuthority_oms.loginId="+accountId+")";
		return tempHQL;
	}
